...
- **Server**: Gunicorn + Nginx (production) / Django dev server (local)
- **Email**: Brevo (Sendinblue) SMTP — branded HTML notifications
- **Python Version**: 3.12+

---

## 🚀 Production Deployment (Ubuntu VPS + Nginx + Gunicorn)

This system is live at **https://tickets.innbucks.org**, deployed on a Ubuntu 24.04 VPS.
...
```bash
git clone https://github.com/40551539/ICT-SUPPORT-SYSTEM.git /var/www/tickets
cd /var/www/tickets
bash deploy.sh
```
...
- Load default ticket categories
- Configure and start a `systemd` service (`tickets.service`)
- Configure Nginx as a reverse proxy

### Environment Variables

After deployment, add credentials to `/var/www/tickets/.env`:
...
### Fix File Permissions (SQLite)

If you see "attempt to write a readonly database":
```bash
chown -R www-data:www-data /var/www/tickets
chmod 664 /var/www/tickets/db.sqlite3
chmod 775 /var/www/tickets
systemctl restart tickets
```
...
certbot --nginx -d tickets.innbucks.org --non-interactive --agree-tos -m your@email.com
```

### Update the Live Site

```bash
git config --global --add safe.directory /var/www/tickets
cd /var/www/tickets && git pull && python manage.py migrate && systemctl restart tickets
```
...
| Command | Purpose |
|---------|---------|
| `systemctl status tickets` | Check if the app is running |
| `systemctl restart tickets` | Restart after config changes |
| `journalctl -u tickets -n 50` | View recent application logs |
...
Nginx (port 80/443)          ← handles SSL, static files
   │
   ▼
Gunicorn (127.0.0.1:8000)    ← 2 sync workers
   │
   ▼
Django (WSGI)                ← config/wsgi.py
   │
   ▼
SQLite / PostgreSQL           ← /var/www/tickets/db.sqlite3
```
